...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(text1, 100, middle - (bound.top + bound.bottom) / 2F, paint2);

        paint2.getTextBounds(text2, 0, text2.length(), bound);
        canvas.drawText(text2, 200, middle - (bound.top + bound.bottom) / 2F, paint2);

        paint2.getTextBounds(text3, 0, text3.length(), bound);
        canvas.drawText(text3, 300, middle - (bound.top + bound.bottom) / 2F, paint2);

        paint2.getTextBounds(text4, 0, text4.length(), bound);
        canvas.drawText(text4, 400, middle - (bound.top + bound.bottom) / 2F, paint2);

        paint2.getTextBounds(text5, 0, text5.length(), bound);
        canvas.drawText(text5, 500, middle - (bound.top + bound.bottom) / 2F, paint2);

        paint2.getTextBounds(text6, 0, text6.length(), bound);
        canvas.drawText(text6, 600, middle - (bound.top + bound.bottom) / 2F, paint2);
    }
}